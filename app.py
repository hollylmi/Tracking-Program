...
with app.app_context():
    ...
    for stmt in [
        ...
        # Production line activity type (deploy/weld) and weld tracking
        "ALTER TABLE entry_production_line ADD COLUMN activity_type VARCHAR(50) DEFAULT 'deploy'",
        "ALTER TABLE entry_production_line ADD COLUMN weld_metres FLOAT DEFAULT 0",
        ...
        # ── 2026-03-30: Transfer — batch + pre/post checks ──
        "ALTER TABLE machine_transfer ADD COLUMN batch_id INTEGER",
        ...
        # ── 2026-04-14: NFC scan location tracking ──
        "ALTER TABLE machine ADD COLUMN last_scanned_at TIMESTAMP",
        'ALTER TABLE machine ADD COLUMN last_scanned_by_user_id INTEGER REFERENCES "user"(id)',
        "ALTER TABLE machine ADD COLUMN last_scanned_lat FLOAT",
        "ALTER TABLE machine ADD COLUMN last_scanned_lng FLOAT",
        "ALTER TABLE machine ADD COLUMN last_scanned_address VARCHAR(500)",
...
def require_login():
    ...
    if request.path.startswith('/equipment/scan/'):
        return  # Scan page self-redirects to /e/<id> for unauth'd viewers
